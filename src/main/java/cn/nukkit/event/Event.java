 ...
 * Things that happens in the server is called a <b>event</b>. Define a
 * procedure that should be executed when a event happens, this procedure is
 * called a <b>listener</b>.
 * </p>
 *
 * <p>
 * Nukkit调用事件的处理器时，会通过参数的类型判断需要被监听的事件。<br>
 * When Nukkit is calling a handler, the event needed to listen is judged by the
